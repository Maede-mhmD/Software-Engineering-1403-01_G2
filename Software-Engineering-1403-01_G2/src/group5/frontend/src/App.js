...
const AutocompleteInterface = () => {
  ...
  const generateNGramSuggestions = (text, n = 2) => {
    const words = text.split(' ');
    const suggestions = [];

    for (let i = 0; i <= words.length - n; i++) {
      const ngram = words.slice(i, i + n).join(' ');
      suggestions.push(ngram);
    }
  ...
  const fetchSuggestions = async (text) => {
    try {
      const response = await fetch(`http://localhost:8000/group5/suggest/?text=${encodeURIComponent(text)}`);
      const data = await response.json();
      const apiSuggestions = data.suggestions || [];
      const ngramSuggestions = generateNGramSuggestions(text);
      const combinedSuggestions = [...new Set([...apiSuggestions, ...ngramSuggestions])];
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    if (suggestion.includes(' ')) {
      const ngramWords = suggestion.split(' ');
      words.splice(words.length - 1, 1, ...ngramWords);
      setInput(words.join(' ') + ' ');
    } else {
